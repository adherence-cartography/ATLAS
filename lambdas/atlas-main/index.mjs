...
import { handleIntegrationRoutes, getOTPAttempts, incrementOTPAttempts, clearOTPAttempts, isOTPLocked } from './lambda_integrations.mjs';

// ── AWS clients ───────────────────────────────────────────────────────────────
const ssm    = new SSMClient({ region: 'us-east-1' });
const ses    = new SESClient({ region: 'us-east-1' });
// UAE SSM — me-central-1. Workspace params for UAE-resident workspaces (ALTHIQA*)
// are stored here to satisfy UAE PDPL data residency requirements.
const _ssmUAE = new SSMClient({ region: 'me-central-1' });

// Route workspace-key SSM operations to the correct region.
// ALTHIQA* keys → me-central-1 (UAE); everything else → us-east-1.
function _ssmForKey(key) {
  return (key && key.toUpperCase().startsWith('ALTHIQA')) ? _ssmUAE : ssm;
}
...
async function lookupWorkspaceKey(key) {
  ...
  const paramName = '/atlas/workspaces/' + key;
  const primarySSM = _ssmForKey(key);
  try {
    const res = await primarySSM.send(new GetParameterCommand({ Name: paramName, WithDecryption: true }));
    const profile = JSON.parse(res.Parameter.Value);
    _wsCache.set(key, profile);
    return profile;
  } catch(e) {
    // During SSM migration: ALTHIQA params may not yet be in me-central-1.
    // Fall back to us-east-1 so the transition is transparent.
    if (primarySSM !== ssm) {
      try {
...
async function handleVerifyOTP(body, origin) {
  ...
  // BP-SEC-01: Check per-session attempt counter before touching the session
  const attempts = await getOTPAttempts(session_token);
  if (isOTPLocked(attempts)) {
    return respond(429, { valid: false, error: 'Too many incorrect attempts. Please wait 15 minutes and enter your key again.' }, origin);
  }
  ...
  if (Date.now() > session.expiresAt) {
    ...
    await clearOTPAttempts(session_token);
  ...
  if (!match) {
    const newAttempts = await incrementOTPAttempts(session_token, attempts.count + 1 >= 3);
    const remaining   = Math.max(0, 3 - newAttempts.count);
    const errMsg      = remaining > 0
      ? `Incorrect code. ${remaining} attempt${remaining === 1 ? '' : 's'} remaining.`
      : 'Too many incorrect attempts. Please wait 15 minutes and try again.';
    return respond(200, { valid: false, error: errMsg, locked: remaining === 0 }, origin);
  }

  // Correct — consume session and clear attempt counter
  await deleteOTPSession(session_token);
  await clearOTPAttempts(session_token);
...
export async function handleIssueKey(body, origin) {
  ...
  const profile = {
    ...
    color: '#8b6ff5',
    // UAE PDPL: ALTHIQA workspaces skip Firebase entirely from day one.
    ...(key.startsWith('ALTHIQA') ? { dyna_only: true } : {}),
  };
  await _ssmForKey(key).send(new PutParameterCommand({
...
export { ssm, _ssmUAE, _ssmForKey, ses, FIREBASE_DB_URL, SES_FROM_EMAIL, VERIFY_BASE_URL, LETTER_TIERS,
...
export async function updateWorkspaceProfile(key, updates) {
  const normalizedKey = key.trim().toUpperCase();
  const regionSSM = _ssmForKey(normalizedKey);
  const existing = await regionSSM.send(new GetParameterCommand({
    Name: `/atlas/workspaces/${normalizedKey}`, WithDecryption: true,
  }));
  const profile = JSON.parse(existing.Parameter.Value);
  const updated  = { ...profile, ...updates };
  await regionSSM.send(new PutParameterCommand({
...
export const handler = async (event) => {
  ...
  if (path.endsWith('/verify-otp') && isRateLimited(clientIp, 'verify-otp', 5, 60_000)) {
    return respond(429, { error: 'Too many OTP attempts — please wait before trying again' }, origin);
  }
  ...
  if (path.startsWith('/create-checkout-session') || path.startsWith('/send-magic-link') ||
      ...
      path.startsWith('/pub-privacy-checkout')) {
    return await handleStripeRoutes(path, method, event.body, event.headers);
  }

  // Integration routes (REDCap, FHIR webhooks, institution provisioning — BP-INT-02/03, BP-FOCUS-02)
  const integrationResult = await handleIntegrationRoutes(path, method, body, origin, event);
...
async function handleAdminCreateKey(body, origin, event) {
  ...
  const profile = {
    ...
    // UAE PDPL: ALTHIQA workspaces skip Firebase entirely from day one.
    ...(profile.key && profile.key.startsWith('ALTHIQA') ? { dyna_only: true } : {}),
  ...
  try {
    await _ssmForKey(profile.key).send(new PutParameterCommand({
...
async function handleAdminRevokeKey(body, origin, event) {
  ...
  const normalizedKey = key.trim().toUpperCase();
  try {
    const _rsmR = _ssmForKey(normalizedKey);
    const existing = await _rsmR.send(new GetParameterCommand({ Name: `/atlas/workspaces/${normalizedKey}`, WithDecryption: true }));
    const profile = JSON.parse(existing.Parameter.Value);
    profile.active = false;
    profile.revoked_at = new Date().toISOString();
    await _rsmR.send(new PutParameterCommand({ Name: `/atlas/workspaces/${normalizedKey}`, Value: JSON.stringify(profile), Type: 'String', Overwrite: true }));
...
async function handleAdminEditKey(body, origin, event) {
  ...
  const normalizedKey = key.trim().toUpperCase();
  const _editSSM = _ssmForKey(normalizedKey);
  try {
    const existing = await _editSSM.send(new GetParameterCommand({ Name: `/atlas/workspaces/${normalizedKey}`, WithDecryption: true }));
    const profile = JSON.parse(existing.Parameter.Value);
    ...
    await _editSSM.send(new PutParameterCommand({
      Name: `/atlas/workspaces/${normalizedKey}`,
      Value: JSON.stringify(profile),
      Type: 'String', Overwrite: true,
    }));
    _wsCache.delete(normalizedKey);
    // Sync to Firebase workspace cache via service-account token so the display
    // email cannot revert on reload due to client-side auth failures.
    try {
      const fbToken  = await mintFirebaseToken('system_admin', { role: 'superadmin' });
      const fbIdTok  = await exchangeCustomTokenForIdToken(fbToken);
      const wsUrl    = `${FIREBASE_DB_URL}/workspaces/${normalizedKey}.json?auth=${fbIdTok}`;
      const existing = (await firebaseRestGet(wsUrl)) || {};
      const wsUpdate = {
        ...existing,
        name:               profile.name,
        ...
        peacs_dims:         profile.peacs_dims          || ['base','mvmt','strata'],
        ...(profile.email       ? { email:       profile.email       } : {}),
        ...(profile.study_title ? { study_title: profile.study_title } : {}),
        ...(profile.role        ? { role:        profile.role        } : {}),
      };
      await firebaseRestPut(wsUrl, wsUpdate);
      console.log('[admin/edit-key] Firebase workspace synced:', normalizedKey);
    } catch (fbErr) {
      console.error('[admin/edit-key] Firebase sync non-fatal:', fbErr.message);
    }
...
async function handleAdminDeleteKey(body, origin, event) {
  ...
  const normalizedKey = key.trim().toUpperCase();
  try {
    await _ssmForKey(normalizedKey).send(new DeleteParameterCommand({ Name: `/atlas/workspaces/${normalizedKey}` }));
    _wsCache.delete(normalizedKey);
...
async function handleInstProvisionSeat(body, origin) {
  ...
  const seatProfile = {
    ...
    // UAE PDPL: seats under an ALTHIQA institution inherit dyna_only from day one.
    ...(seatKey.startsWith('ALTHIQA') ? { dyna_only: true } : {}),
  };

  try {
    await _ssmForKey(seatKey).send(new PutParameterCommand({
...
// ══════════════════════════════════════════════════════════════════════════════
// Data-Privacy Publication License — webhook issuance (called from Stripe webhook)
// ══════════════════════════════════════════════════════════════════════════════
