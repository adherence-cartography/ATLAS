...
${_fsTable(['Jurisdiction','Framework','Cross-Border Transfer to USA','ATLAS Status'],[
  ...
  ['<strong>Brazil</strong>','LGPD 2020 (Lei 13.709/2018)','Permitted via Standard Contractual Clauses or local data residency.','LGPD-compliant. Brazilian institutional workspaces (region: br) use AWS DynamoDB sa-east-1 (São Paulo) exclusively — no PHI leaves Brazil. LGPD Art. 33 cross-border transfer obligation satisfied by local residency. Global workspaces: Firebase LGPD DPA terms apply.'],
...
<div style="font-family:'Courier New',monospace;font-size:9px;color:#9ca3af;margin-bottom:20px">Standard Subscription — Direct Assessment Workflow · Version 2026.8 · ${today}</div>
...
${_fsSection(6,'Data Storage',_fsTable(['Property','Detail'],[
  ...
  ['Brazil workspace database','AWS DynamoDB — sa-east-1 (São Paulo, Brazil). Brazilian institutional workspaces (region: br) write exclusively to sa-east-1; no PHI leaves Brazil. See §9.'],
  ['Encryption at rest','AES-256 — Firebase: Google-managed keys; DynamoDB (UAE, Brazil, EU): AWS-managed keys. Both enforce encryption at the storage layer.'],
...
${_fsSection(7,'Third-Party Data Processors',_fsTable(['Service','Provider','Purpose','Data Received'],[
  ...
  ['AWS DynamoDB + Lambda','Amazon Web Services (Brazil — sa-east-1, São Paulo)','Brazil workspace data storage and compute','Brazilian institutional workspace assessment records, PEACS, audit log. No data from global workspaces.'],
...
${_fsIntlAddendum()}
<div style="border-top:1px solid #e5e7eb;padding-top:14px;margin-top:36px;font-family:'Courier New',monospace;font-size:9px;color:#9ca3af;display:flex;justify-content:space-between"><span>ATLAS · Adherence Inc. · Philip Morisky, MBA · info@adherence.cc · adherence.cc</span><span>ATLAS v8.9.3 · August 2026</span></div>`);
}
...
function _fsBulk(){
  ...
  return _applyFsTheme(`<div style="font-family:'Courier New',monospace;font-size:9px;letter-spacing:0.18em;text-transform:uppercase;color:#7a8b9a;margin-bottom:10px">ATLAS Platform · Adherence Inc.</div>
<h1 style="font-size:22px;font-weight:400;color:#0f1e33;margin-bottom:4px">Data Security Fact Sheet</h1>
<div style="font-family:'Courier New',monospace;font-size:9px;color:#9ca3af;margin-bottom:20px">Researcher Subscription — De-Identified Excel Bulk Upload Workflow · Version 2026.8 · ${today}</div>
<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;padding:12px 16px;margin-bottom:28px;display:flex;align-items:flex-start;gap:12px;">
  <span style="font-size:1rem;flex-shrink:0;margin-top:1px;">🔒</span>
  <div>
    <div style="font-family:'Courier New',monospace;font-size:8.5px;letter-spacing:0.12em;text-transform:uppercase;color:#15803d;font-weight:600;margin-bottom:5px;">HIPAA Infrastructure · 21 CFR Part 11 · Compliance Status</div>
...
${_fsSection(5,'Data Storage',_fsTable(['Property','Detail'],[
  ...
  ['Brazil workspace database','AWS DynamoDB — sa-east-1 (São Paulo, Brazil). Brazilian institutional workspaces (region: br) write exclusively to sa-east-1; no PHI leaves Brazil. See §8.'],
  ['Encryption at rest','AES-256 — Firebase: Google-managed keys; DynamoDB (UAE, Brazil, EU): AWS-managed keys. Both enforce encryption at the storage layer.'],
  ['Encryption in transit','TLS 1.3 / TLS 1.2 — all connections HTTPS. HSTS preload enforced at the Cloudflare edge.'],
...
${_fsSection(8,'Data Residency & Regulatory Framework',`<p style="margin-bottom:10px;color:#374151"><strong>Global workspaces:</strong> All data stored in the <strong>United States</strong> on Google Cloud (Firebase, us-central1). Firebase GDPR DPA available at firebase.google.com/support/privacy.</p><p style="margin-bottom:10px;color:#374151"><strong>UAE workspaces:</strong> All assessment records and audit logs are stored exclusively in <strong>AWS DynamoDB — me-central-1 (Abu Dhabi, UAE)</strong>. Data routes via the Cloudflare Worker to a Lambda in me-central-1 and never transits or is stored in US infrastructure. This satisfies UAE PDPL (Federal Decree-Law No. 45 of 2021) data localisation requirements and ADHICS v2.0.</p><p style="margin-bottom:10px;color:#374151"><strong>Brazilian institutional workspaces (region: br):</strong> All assessment records, PEACS scores, and audit logs are stored exclusively in <strong>AWS DynamoDB — sa-east-1 (São Paulo, Brazil)</strong>. Data routes via the Cloudflare Worker to a Lambda in sa-east-1 and never transits or is stored in US infrastructure. This satisfies Brazil LGPD (Lei 13.709/2018) Article 33 data localisation requirements by eliminating cross-border transfer entirely. dyna_only:true is set at workspace creation — Firebase receives no PHI from Brazilian institutional clients.</p><p style="color:#374151">Researchers with in-country residency requirements outside the US, UAE, or Brazil should consult their data protection officer. See the International Research Addendum (Section A) for jurisdiction-specific guidance.</p>`)}
